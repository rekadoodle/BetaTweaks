...
public class Config {
	...
	public void writeConfig() {
		Utils.openFile(configFile);
		Utils.printLn("#Config file for Beta Tweaks");
		Utils.printLn();
		Utils.printLn("#Clientside Settings");
		printSettings(optionsClient);
		Utils.printLn();
		Utils.printLn("#Singleplayer Only Settings");
		printSettings(optionsGameplay);
		Utils.closeFile();
	}
	
	private void printSettings(SBase<?>[] settings) {
		for(SBase<?> setting : settings) {
			Utils.print(setting.name + "=" + setting.getValue());
			String s = SettingInfo.getConfigComment(setting);
			if(s != null) {
				Utils.print(" #" + s);
			}
			Utils.printLn();
	...
	public SBoolean showAllResolutionsInConsole = new SBoolean("showAllResolutionsInConsole", false);
	...
	public final SBase<?>[] optionsClient = {
			...
			customFullscreenRes, showAllResolutionsInConsole
	};
